...
while gradient_norm > tolerance
    ...
    % Calculate bias terms
    b_1 = ones(500,1)*w1_b; % 50X3
    b_2 = ones(500,1)*w2_b; % 50X1
    % Calculate the output
    a_1 = X_norm*w1 + b_1; % 50X3
    z_1 = h_relu(a_1); % Using tanh activation function (input layer) % 50X3
    a_2 = z_1*w2 + b_2; % 50X1
    ...
    % Deltas required for gradient calculation
    d_3 = J(z_2,T); % 50X1
    d_2 = d_3*w2'.*h_relu_derivative(a_1); %could be z_1 or a_1 most likely a_1 for relu only % Is this correct? % 50X3
    % Gradients for w1 and w2
...
while gradient_norm > tolerance
    ...
    % Calculate bias terms
    b_1 = ones(5000,1)*w1_b; % 50X3
    b_2 = ones(5000,1)*w2_b; % 50X1
    % Calculate the output
    a_1 = X_norm*w1 + b_1; % 50X3
    z_1 = h_relu(a_1); % Using tanh activation function (input layer) % 50X3
    a_2 = z_1*w2 + b_2; % 50X1
    ...
    % Deltas required for gradient calculation
    d_3 = J(z_2,T); % 50X1
    d_2 = d_3*w2'.*h_relu_derivative(a_1); % Is this correct? % 50X3
    % Gradients for w1 and w2
    del1 = X_norm'*d_2; % 1X3 (w1 matrix gradients w.r.t cost) % THIS PART IS WRONG IN PART 1 !!! CHANGE a_1 to DATA! TRY AGAIN!
    del2 = z_1'*d_3; % 3X1 (w2 matrix gradients w.r.t cost)
    % Gradients for offset bias terms w1_b and w2_b
